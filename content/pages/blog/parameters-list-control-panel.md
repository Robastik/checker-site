...
Как правило, для аналитики используется скользящее окно времени - предыдущий день, последние 7 или 30 дней. Чтобы избавить пользователя от высчитывания дат параметр функции использует не саму дату, а количество дней до нее. То есть предыдущий день - это не дата в формате ДД.ММ.ГГГГ, а 1 - то есть один день назад от текущей даты. В некоторых случаях период берется включая предыдущий день (параметр 1), в других - включая текущий день (параметр 0). То есть период последних семи дней исключая текущий: первый параметр → 8, второй → 1.
